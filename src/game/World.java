    ...
    public static void initializeRooms()
    {
        ...
        World.addRooms(new Room[]{
                new Room("Cyro Pod Room",
                         "The room around you is eerily quiet. A dead alien sits in the opposite side of the room from the broken pod from which you awoke. There is also a door to the east of the room",
                         new Point(0, 1),
                         new ArrayList<>(),
                         new HashMap<String, Command<?>>() {{
                             put("investigate alien", new Command<Void>("investigate alien", args -> {
                                 if (!Player.getCurrentRoom().getRoomCommands().containsKey("alien_investigated")) {
                                     Player.getCurrentRoom().addCommand("alien_investigated", new Command<Void>("alien_investigated", a -> CommandResult.success(null, "")));
                                     if (Player.addItem(new Item("Keycard",
                                         ...
                                         return CommandResult.success(null, "You approach the dead alien. Upon closer inspection, you notice it's Dr. Grobu. His skin has turned a sickly green color, and his body appears to have mutated significantly. This must be the effects of the disease he mentioned in his letter. You find a keycard on his body and add it to your inventory.");
                                     }
                                     return CommandResult.fail("You found Dr. Grobu's keycard but couldn't pick it up. Your inventory might be full.");
                                 }
                                 return CommandResult.success(null, "You approach the dead alien. Upon closer inspection, you notice it's Dr. Grobu. His skin has turned a sickly green color, and his body appears to have mutated significantly. This must be the effects of the disease he mentioned in his letter.");
                             }));
                         }},
                         null),
                new Room("Lab",
                ...
                new Room("Gas Room",
                         ...
                         new HashMap<String, Command<?>>() {{
                             ...
                             put("investigate vault", new Command<Void>("investigate vault", args -> {
                                 ...
                                 Player.getCurrentRoom().addCommand("put in combo", new Command<String>("put in combo", comboArgs -> {
                                     if (!Player.getCurrentRoom().getRoomCommands().containsKey("combo_attempted")) {
                                         if (comboArgs.length < 1) {
                                             return CommandResult.fail("Please enter the combination in the format: put in combo 23-45-89");
                                         }
                                         
                                         String combo = comboArgs[0].trim();
                                         if (combo.equals("23-45-89")) {
                                             Player.getCurrentRoom().addCommand("combo_attempted", new Command<Void>("combo_attempted", a -> CommandResult.success(null, "")));
                                             return CommandResult.success(null, "You input the combination '23-45-89'. The vault door clicks and slowly swings open, revealing the secure storage area inside. You can now access the Clowth gas.");
                                         }
                                         return CommandResult.fail("Incorrect combination. The keypad beeps in rejection.");
                                     }
                                     return CommandResult.fail("You've already unlocked the vault.");
                                 }));
                                 return CommandResult.success(null, "You approach the vault door. It's a heavy-duty security door with a digital keypad. There's a small terminal next to it that might have some useful information.");
                             }));
                ...
                new Room("Genetic Testing Room",
                         "As you walk in you see a large animal on the ground in front of you glowing bright orange. To your right you see 2 cages. One with a red animal, and another one with a blue animal. To your left you see 2 cages, one with a green animal and the other one busted open. On the back of the wall you see a computer terminal.",
                         new Point(1, 0),
                         ...
                         new HashMap<String, Command<?>>() {{
                             ...
                             put("investigate computer terminal", new Command<Void>("investigate computer terminal", args -> {
                                 ...
                                 Player.getCurrentRoom().addCommand("put in password", new Command<String>("put in password", passwordArgs -> {
                                     if (!Player.getCurrentRoom().getRoomCommands().containsKey("password_attempted")) {
                                         if (passwordArgs.length < 1) {
                                             return CommandResult.fail("Please enter a password after the command. Example: put in password mirequills");
                                         }
                                         
                                         String password = passwordArgs[0].toLowerCase();
                                         if (password.equals("mirequills")) {
                                             ...
                                             if (Player.addItem(new Item("Sheers",
                                                 ...
                                                 return CommandResult.success(null, "The screen flashes green and a nearby drawer slides open, revealing a pair of sheers. You've added the sheers to your inventory.");
                                             }
                                             return CommandResult.fail("Found the sheers but couldn't pick them up. Your inventory might be full.");
                                         }
                                         return CommandResult.fail("Incorrect password. The terminal beeps in rejection.");
                                     }
                                     return CommandResult.fail("You've already accessed the terminal and retrieved the sheers.");
                                 }));
                                 return CommandResult.success(null, "You approach the computer terminal. The screen displays a login prompt. There's a sticky note nearby with what appears to be a password hint: 'What's orange and deadly? (lowercase)'");
                             }));
                ...
                new Room("Final Mixing Room",
                         ...
                         new HashMap<String, Command<?>>() {{
                             put("investigate tube 1", new Command<Void>("investigate tube 1", args -> {
                                 Player.getCurrentRoom().addCommand("put in tube 1", new Command<String>("put in tube 1", tubeArgs -> {
                                     if (!Player.getCurrentRoom().getRoomCommands().containsKey("tube1_filled")) {
                                         if (tubeArgs.length < 1) {
                                             return CommandResult.fail("Please specify what to put in tube 1. Example: put in tube 1 Clowth");
                                         }
                                         String item = tubeArgs[0].trim();
                                         if (item.equalsIgnoreCase("Clowth")) {
                                             Player.getCurrentRoom().addCommand("tube1_filled", new Command<Void>("tube1_filled", a -> CommandResult.success(null, "")));
                                             return CommandResult.success(null, "You place the Clowth gas into tube 1. The tube begins to glow with a soft blue light.");
                                         }
                                 ...
                                 return CommandResult.success(null, "You examine tube 1. It appears to be designed for gaseous substances.");
                             }));
                             
                             put("investigate tube 2", new Command<Void>("investigate tube 2", args -> {
                                 Player.getCurrentRoom().addCommand("put in tube 2", new Command<String>("put in tube 2", tubeArgs -> {
                                     if (!Player.getCurrentRoom().getRoomCommands().containsKey("tube2_filled")) {
                                         if (tubeArgs.length < 1) {
                                             return CommandResult.fail("Please specify what to put in tube 2. Example: put in tube 2 Mirequills Fur");
                                         }
                                         String item = String.join(" ", tubeArgs).trim();
                                         if (item.equalsIgnoreCase("Mirequills Fur")) {
                                             Player.getCurrentRoom().addCommand("tube2_filled", new Command<Void>("tube2_filled", a -> CommandResult.success(null, "")));
                                             return CommandResult.success(null, "You place the Mirequills Fur into tube 2. The tube begins to pulse with an orange glow.");
                                         }
                                         return CommandResult.fail("That item doesn't belong in this tube.");
                                     }
                             ...
                             put("investigate vaccine maker", new Command<Void>("investigate vaccine maker", args -> {
                                 if (Player.getCurrentRoom().getRoomCommands().containsKey("tube1_filled") && 
                                     Player.getCurrentRoom().getRoomCommands().containsKey("tube2_filled")) {
                                     Player.getCurrentRoom().addCommand("activate machine", new Command<String>("activate machine", passwordArgs -> {
                                         if (!Player.getCurrentRoom().getRoomCommands().containsKey("machine_activated")) {
                                             if (passwordArgs.length < 1) {
                                                 return CommandResult.fail("Please enter the activation code. Example: activate machine CURE2024");
                                             }
                                             String password = passwordArgs[0].toUpperCase();
                                             if (password.equals("CURE2024")) {
                                                 Player.getCurrentRoom().addCommand("machine_activated", new Command<Void>("machine_activated", a -> CommandResult.success(null, "")));
                                                 if (Player.addItem(new Item("Cure",
                                                     "A glowing vial containing the cure for the mutation disease. This should save the planet."))) {
                                                     return CommandResult.success(null, "The machine whirs to life, combining the ingredients. After a few moments, a small vial containing a glowing liquid emerges. You've created the cure!");
                                                 }
                                                 return CommandResult.fail("Created the cure but couldn't pick it up. Your inventory might be full.");
                                             }
                                     ...
                                     return CommandResult.success(null, "The vaccine maker is ready to combine the ingredients. You notice a keypad with the text 'Enter activation code: CURE____'");
                                 }
                                 return CommandResult.success(null, "The vaccine maker appears to be waiting for all ingredients to be placed in the tubes before it can be activated.");
                             }));
                         }},
                         null),
                new Room("Yard",
